...
TEST(nearest_neighbor_elements, check_int32_t) {
    ...
    for (int i = 0; i < in.size(); i++) { in[i] = 2 * i; }
    in[234] = 0; in[235] = 1;
...
TEST(nearest_neighbor_elements, check_double) {
    ...
    EXPECT_NEAR(out_index[0], 189, 1e-6);
    EXPECT_NEAR(out_index[1], 190, 1e-6);
}
...
TEST(nearest_neighbor_elements, check_float) {
    ...
    EXPECT_NEAR(out_index[0], 0, 1e-4);
    EXPECT_NEAR(out_index[1], 1, 1e-4);
}
