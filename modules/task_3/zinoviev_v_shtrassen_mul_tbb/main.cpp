...
double* shtrassen_mul_tbb(double* A, double* B, int n) {
  if (n > 64) {
    int size = n * n, n_2 = n / 2, size_4 = size / 4;
    double ** A_ = new double*[4];
    double ** B_ = new double*[4];
    ...
    g.run([&] {P[0] = shtrassen_mul(matrix_sum(A_[0], A_[3], size_4),
      matrix_sum(B_[0], B_[3], size_4), n_2); });
...
double* expand_shtrassen_tbb(double* A, double* B, int n) {
  if (n > 64) {
    int check_pow[8] = { 128, 256, 512, 1024, 2048, 4096, 8192, 16384 };
    int new_size = 0;
    for (int i = 0; i < n; ++i) {
      if (check_pow[i] == n) {
        return shtrassen_mul_tbb(A, B, n);
      }
      if (n < check_pow[i]) {
        new_size = check_pow[i];
        break;
...
int main(int argc, char** argv) {
  ...
  if (check(res_mul, res_shtrass_seq, size, &bad1, eps) &&
    ...
    if (bad1 != -1) {
      ...
      if (bad2 != -1) {
        std::cout << "bad multiplication omp! " << bad2 << " elem: "
          << res_mul[bad2] << "!=" << res_shtrass_omp[bad2] << std::endl << std::endl;
      } else {
        std::cout << "bad multiplication tbb! " << bad3 << " elem: "
          << res_mul[bad3] << "!=" << res_shtrass_omp[bad3] << std::endl << std::endl;
      }
    }
