...
int main(int argc, char** argv) {
  ...
  if (check(res_mul, res_shtrass_seq, size, &bad, eps))
    ...
    std::cout << "bad multiplication! " << bad << " elem: "
    << std::fixed << res_mul[bad] << "!="
    << std::fixed << res_shtrass_seq[bad] << std::endl;
}
