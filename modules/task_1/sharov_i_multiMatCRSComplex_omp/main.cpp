...
void InitializeMatrix(int N, int M, int NZ, Matrix &mat)
{
    mat.N = N;
    mat.M = M;
    mat.NZ = NZ;
...
void GetMatrix(int N, int M, int countinrow, Matrix &mat)
{
    ...
    notnull = countinrow*N;

    InitializeMatrix(N, M, notnull, mat);
...
void multiplication(Matrix &A, Matrix &BT, Matrix &C) {
    double t1, t2;
    ...
    InitializeMatrix(A.N, BT.N, columns.size(), C);
...
int main(int argc, char* argv[]) {
    ...
    printf("Zadacha 1. The multiplication of sparse matrices. Elements of the complex type. matrix storage format - string (CRS):\n");
    ...
    InitializeMatrix(m2, n2, nzInRow2*n2, BT);
    Transposing(B, BT);
    multiplication(A, BT, C);
