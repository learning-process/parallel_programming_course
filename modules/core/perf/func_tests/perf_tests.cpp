...
TEST(perf_tests, check_print_perf_negative) {
  // Create data
  std::vector<float> in(2000, 1);
  std::vector<float> out(1, 0);
  ...
  ASSERT_LE(perfResults->time_sec, 10.0);
  EXPECT_THROW(ppc::core::Perf::print_perf_statistic(perfResults), std::runtime_error);
}

TEST(perf_tests, check_print_perf_task) {
  // Create data
  std::vector<float> in(2000, 1);
  std::vector<float> out(1, 0);
  ...
  // Create Task
  auto testTask = std::make_shared<ppc::test::TestTask<float>>(taskData, true);
...
TEST(perf_tests, check_print_perf_pipeline) {
  // Create data
  std::vector<float> in(2000, 1);
  std::vector<float> out(1, 0);
