...
1. **Configure the build**: ``Makefile``, ``.sln``, etc.
   ...
   .. code-block:: bash
      ...
      cmake -S . -B build -DPPC_TASKS=example -D USE_FUNC_TESTS=ON -D USE_PERF_TESTS=ON -D CMAKE_BUILD_TYPE=Release
   ...
   *Help on CMake keys:*
   ...
   - ``-D PPC_TASKS=all`` builds every task (default). Pass one task or a semicolon list,
     for example ``-D PPC_TASKS=example``, to limit the build. Meta-layout tasks
     with ``threads`` and ``processes`` subtrees are built as one selected task.
